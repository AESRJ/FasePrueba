...
class Player(pygame.sprite.Sprite):
    def __init__(self, x, y, is_echo=False):
        ...
        try:
            ...
            self.animations['run'] = [run_scaled1, idle_scaled, run_scaled3, idle_scaled]
        ...
        if self.is_echo:
            self.tint_images()
            from .mechanics import GhostPlatform 
            self.ghost_platform = GhostPlatform()

    def tint_images(self):
        for key, value in self.animations.items():
            if isinstance(value, list):
                tinted_list = []
                for img in value: tinted_list.append(self.apply_tint(img))
    ...
    def animate(self):
        ...
        if not self.on_ground: img = self.animations['jump']
            ...
            img = self.animations['idle']
            if self.walk_frame != 0: self.walk_frame = 0
    ...
    def update(self, platforms, input_active=True):
        if not self.is_echo:
            # Lógica Normal
            if input_active: self.handle_input()
            ...
            # --- CAPTURAR DATOS DE LA PLATAFORMA ---
            ghost_data = None
            
            # Revisamos qué hay debajo
            self.rect.y += 2 # Extendemos un poco para detectar contacto firme
            hits = pygame.sprite.spritecollide(self, platforms, False)
            self.rect.y -= 2
            
            for hit in hits:
                if hasattr(hit, 'type') and hit.type == 'gate':
                    # Guardamos (x, y, w, h) exactos de la plataforma en este momento
                    ghost_data = (hit.rect.x, hit.rect.y, hit.rect.width, hit.rect.height)
                    break
            ...
            frame_data = {
                ...
                'ghost_data': ghost_data # Guardamos los datos completos
            }
            ...
            if self.playback_index < len(self.recording):
                ...
                # --- ENVIAR DATOS A LA PLATAFORMA FANTASMA ---
                ghost_data = data.get('ghost_data')
                self.ghost_platform.update_state(ghost_data)
                
                self.animate()
                self.playback_index += 1
            else:
                self.finished_playback = True
                self.ghost_platform.update_state(None) # Ocultar
